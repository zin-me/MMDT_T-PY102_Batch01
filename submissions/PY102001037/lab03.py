...
def insert_linear_probing(table: list[int | None], key: int) -> list[int | None]:
    ...
    for _ in range(size):
        if table[index] is None:
            ...
            return table
        index = (index + 1) % size
    raise Exception("Hash table is full Now!")
...
def insert_quadratic_probing(table: list[int | None], key: int) -> list[int | None]:
    ...
    hash_index = key % size
    for i in range(size):
        index = (hash_index + i * (i + 1) // 2) % size # Since table size is 4(not prime) in the example, I've changed the formula to avoid overwriting the same slot twice. 
        if table[index] is None:
            table[index] = key
            return table

    return table  # If the table is full, return it unchanged