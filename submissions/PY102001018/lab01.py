...
def doubleIt(head):
    ...
    """
    # if the head is 5 or more, the result will need a new leading 1
    if head.val >= 5:
        head = Node (0, head)

    curr = head
    while curr:
        # double current value and keep the last digit
        curr.val = (curr.val * 2) % 10

        # if next digit is 5+, add the carry to this node
