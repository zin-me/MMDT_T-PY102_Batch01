...
def next_greater_to_right(nums: list[int]) -> list[int]:
    ...
    for i in range(n):
        ...
        while temp_stack and current > nums[temp_stack[-1]]:
...
def first_non_repeating(stream: str) -> str:
    ...
    for char in stream:
        ...
        while queue and counts[queue[0]] > 1:
            ...
            ##print(while_counter+1,' while loop')
            #while_counter+=1
...
def hot_potato(names: list[str], k: int) -> str:
    ...
    circle_queue = deque(names)

    while len(circle_queue) > 1:
        
        for i in range(k):
            passed = circle_queue.popleft()
            circle_queue.append(passed)
            
        elminated = circle_queue.popleft()

    return circle_queue[0]
