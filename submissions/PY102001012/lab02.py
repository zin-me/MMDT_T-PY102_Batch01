...
def is_balanced_parentheses(s: str) -> bool:
    ...
    stack_list = []
    for char in s:
        if char in pair_dict.values():  # opening brackets
            stack_list.append(char)
        elif char in pair_dict:  # closing brackets
...
def hot_potato(names: list[str], k: int) -> str:
    ...
    while len(q) > 1:
        for i in range(k):
            q.append(q.popleft())
        q.popleft() #remove 1st of queue
    return q[0] #1 left in queue = winner
