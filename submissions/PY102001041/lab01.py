...
def reverseList(head):
    ...
    # iteration to change pointers direction
    while curr:
        next_node = curr.next
        curr.next = prev
        prev = curr
...
def doubleIt(head):
    ...
    # double reverse LL and keep track of carry
    while current:
    ...
    # add new node if carry exist    
    if carry:
        last_node.next = Node(carry)
    

    # reverse list again
    result = reverseList(reverse_head)
    return result
