...
def insert_linear_probing(table: list[int | None], key: int) -> list[int | None]:
    ...
    for i in range(n):
        # Linear probe: (hash + i) % table_size
        index = (initial_hash + i) % n
        if table[index] is None:
            table[index] = key
            return table
            
    return table  
        # Return table as is if no slots are available
...
def insert_quadratic_probing(table: list[int | None], key: int) -> list[int | None]:
    ...
    for i in range(n):
        # Quadratic probe: (hash + i^2) % table_size
        index = (initial_hash + i * i) % n
        if table[index] is None:
            table[index] = key
            return table
